  ...
  if (req.files) {
    ...
    if (req.files.video && req.files.video[0]) {
      ...
      if (!allowedVideoTypes.includes(videoFile.mimetype)) {
        throw new ApiError(400, `Unsupported video format. Supported formats: MP4, MOV, WebM, AVI. Received: ${videoFile.mimetype}`);
      }
      ...
      try {
        ...
        // Log video upload for debugging
        console.log(`Video uploaded: ${videoUrl}, MIME type: ${videoFile.mimetype}, Size: ${videoFile.size} bytes`);
        
        // Warn about potential ImageKit limitations
        console.warn("⚠️ Video uploaded to ImageKit. Note that free plans have video transformation limits which may prevent playback.");
      } catch (uploadError) {
        console.error("ImageKit video upload error:", uploadError);
        // Check if it's a transformation limit error
        if (uploadError?.response?.headers?.['ik-error']) {
          const ikError = uploadError.response.headers['ik-error'];
          if (ikError.includes('ELIMIT')) {
            console.error("ImageKit ELIMIT error detected - video transformation limit exceeded");
            throw new ApiError(400, "Video upload failed due to ImageKit transformation limits. Please upgrade your ImageKit plan or optimize your video before upload.");
          }
        }
        throw new ApiError(500, "Failed to upload video to ImageKit. Please try again.");
      }
    }
  }
...
const getProducts = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // ✅ If search term matches SKU, prioritize those products
    if (search && search.trim() !== "") {
      const skuRegex = new RegExp(search.trim(), "i");
      const skuMatchProducts = await Product.find({ 
        ...filter, 
        sku: skuRegex 
      })
      .populate("category", "name slug")
      ...
      if (skuMatchProducts.length > 0) {
        // Combine SKU matches with other search results, putting SKU matches first
        const skuProductIds = skuMatchProducts.map(p => p._id.toString());
        const nonSkuProducts = products.filter(p => !skuProductIds.includes(p._id.toString()));
        
        // Prioritize SKU matches
        products = [...skuMatchProducts, ...nonSkuProducts];
      }
    }
    ...
    return res
      ...
      .json(
        new ApiResponse(
          ...
          "Products fetched successfully (mock data)"
        )
      );
  }
});
